    ...
    <q-form @submit="submitForm" @reset="resetForm" class="q-gutter-md">
      <q-card>
        ...
        <q-card-section>
          <div class="q-mb-sm">
            ...
            <q-input id="constituent_name" name="constituent_name" v-model="state.name"
              ...
              :error-message="errorState?.name" @update:model-value="errorState.name = ''" :disable="fetchLoading" hide-bottom-space />
          </div>
          <div class="q-mb-sm">
            ...
            <q-input id="constituent_nik" name="constituent_nik" v-model="state.nik"
              ...
              :error-message="errorState?.nik" @update:model-value="errorState.nik = ''" :disable="fetchLoading" hide-bottom-space />
          </div>
          <div class="q-mb-sm">
            ...
            <q-input id="constituent_phone" name="constituent_phone" v-model="state.phone"
              ...
              :error-message="errorState?.phone" @update:model-value="errorState.phone = ''" :disable="fetchLoading" hide-bottom-space />
          </div>
          <div class="q-mb-sm">
        ...
        <q-card-actions align="between">
          <q-btn type="reset" no-caps size="md" color="warning" flat :to="{ name: 'Constituent Index Page', query: { refresh: backPageNeedRefresh }  }">
            <ph-icon name="ArrowUUpLeft" size="16" class="q-mr-sm" />
            <span>Kembali</span>
          </q-btn>
          <q-btn type="submit" no-caps size="md" color="primary" text-color="dark" :loading="submitLoading || fetchLoading"
            :disable="submitLoading || fetchLoading">
...
const getDetail = async () => {
  ...
  await api.get('v1/constituent/' + id)
    .then(async (res) => {
      ...
      return await api.get('v1/user-area/' + detailData.value.user_area_id)
        .then((res) => {
          console.log('res', res)
